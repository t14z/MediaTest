...
public class MyAudioRecord1 {
    ...
    public MyAudioRecord1() {
    }

    public MyAudioRecord1(MediaMuxer mMediaMuxer) {
    ...
    public void startRecording() {
        try {
            ...
            if (mMediaMuxer == null) {
                ...
                dir.mkdirs();
                String name = mDateTimeFormat.format(new GregorianCalendar().getTime());
                mOutputPath = new File(dir, name + ".mp4").toString();
                ...
                Thread thread = new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        while (isRunning) {
                            ...
                            /***
                             05-10 11:46:58.177: localRequestStop = false   localRequestDrain = false   mRequestDrain = 0
                             05-10 11:46:58.317: localRequestStop = false   localRequestDrain = false   mRequestDrain = 0
                             05-10 11:46:58.487: localRequestStop = false   localRequestDrain = true   mRequestDrain = 0
                             05-10 11:47:01.741: localRequestStop = true   localRequestDrain = true   mRequestDrain = 274
                             **/
                            if (localRequestStop) {
                                drain();
...
//	            if (DEBUG) Log.v(TAG, "encode:queueInputBuffer");
        ...
        while (mIsCapturing) {
            ...
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                if (!Muxer_start()) {
                    ...
                    synchronized (MyAudioRecord1.this) {
                        while (!mIsStarted)
                            try {
                                MyAudioRecord1.this.wait(100);
                            } catch (InterruptedException e) {
                                e.printStackTrace();
                                break LOOP;
                            }
     ...
     */
    /*package*/
    synchronized boolean Muxer_start() {
        mMediaMuxer.start();
        mIsStarted = true;
        notifyAll();
        return mIsStarted;
     ...
     */
    /*package*/
    synchronized void Muxer_stop() {
        //mMediaMuxer.stop();
        //mMediaMuxer.release();
        mIsStarted = false;
        if (muxerStop != null) muxerStop.stop();
    }

    private MuxerStop muxerStop;

    public void setMuxerStop(MuxerStop muxerStop) {
        this.muxerStop = muxerStop;
    }

    public interface MuxerStop {
        public void stop();
    }
