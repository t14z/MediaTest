...
public class EncodeAndMuxTest {
    ...
    public EncodeAndMuxTest(final Context context, List<ImageInfo> imgList, LastImageInfo lastImgInfo) throws Exception {
        if (imgList.size() > 9) {
        ...
        mExplainCubePositions = ByteBuffer.allocateDirect(explainCubePosition.length * EncodeAndMuxTest.BYTES_PER_FLOAT).order(ByteOrder.nativeOrder()).asFloatBuffer();
